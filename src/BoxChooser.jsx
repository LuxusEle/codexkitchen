...
function BoxDialog({p,initial,onClose,onApply}) {
  ...
  return <dialog className="box-dialog" ref={ref} onCancel={onClose} onClose={onClose} aria-labelledby="box-title">
    <div className="box-dialog-heading"><div><p className="eyebrow">CABINET BRIEF · MILLIMETRES</p><h2 id="box-title">Choose your boxes</h2></div><button className="secondary" onClick={onClose} aria-label="Close box chooser"><X size={18}/></button></div>
      ...
      {types.map(type=>{
        ...
        return <div className={`box-choice ${on?'chosen':''}`} key={type}>
          ...
          <div className="box-inputs">
            ...
            <label>Width {FIXED_WIDTH_TYPES.includes(type)&&<small>fixed</small>}<input aria-label={`${t.name} default width`} type="number" step="10" min={minimumCabinetWidth({type})} max={maximumCabinetWidth({type})} disabled={!on||FIXED_WIDTH_TYPES.includes(type)} value={t.w} onChange={e=>dimensions(type,'w',Number(e.target.value))}/></label>
            <label>Height<input aria-label={`${t.name} default height`} type="number" step="10" min={upper?200:t.h>1000?1600:600} max={upper?1200:t.h>1000?3000:900} disabled={!on} value={t.h} onChange={e=>dimensions(type,'h',Number(e.target.value))}/></label>
    ...
    <div className="box-dialog-actions"><button className="secondary" onClick={onClose}>Cancel</button><button className="primary" disabled={plan.errors.length>0||(!acceptUnplaced&&plan.unmet.length>0)} onClick={()=>onApply({...draft,units:plan.units})}><Check size={16}/>OK · apply boxes</button></div>
  </dialog>;
}
