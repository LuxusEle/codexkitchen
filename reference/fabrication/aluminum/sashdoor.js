// sashdoor.js — Complete Sash Door & Hinge Bores Generator

function generateDoor(params) {
  ...
  const profW = params.width || 45;   // Face of the profile in X-Y plane (45mm)
  const profH = params.height || 21.2; // Depth of the profile in Z plane (21.2mm)
  
  const twist = Math.PI / 2; 
  p.flipDepth = false;

  // 1. Miter CSG Function
  function applyMiterCSG(barMeshes, barLength, profWidth, profHeight, scene) {
  ...
  // 2. Hinge CSG Drill Function (35mm Cup Bores)
  function applyHingeCSG(barMeshes, barLength, profWidth, profHeight, bores, scene) {
  ...
  // Helper: Attach Handle Lip (continuous finger pull) if this bar is the handleBar
  function attachHandleLipIfSelected(barObj, barId, barLength, parentNode) {
    if (params.handleBar === barId) {
      const lip = BABYLON.MeshBuilder.CreateBox(barId + '_handle_lip', {
        width: barLength / 1000,
        height: 0.012,
        depth: 0.014
      }, scene);
  ...
  attachHandleLipIfSelected(bot, 'BOT', width, fBot);
  ...
  attachHandleLipIfSelected(rht, 'RHT', height, fRht);
  ...
  attachHandleLipIfSelected(top, 'TOP', width, fTop);
  ...
  attachHandleLipIfSelected(lft, 'LFT', height, fLft);

  // 5. Inset Glass / ACP Panel
  ...
  const panelType = params.panelType || 'glass';
  const panelColorHex = params.panelColor || '#1e293b';
  
  const glassMat = new BABYLON.StandardMaterial('glassMat', scene);
  glassMat.diffuseColor = BABYLON.Color3.FromHexString(panelColorHex);
  
  if (panelType === 'glass') {
      glassMat.alpha = 0.5;
  } else {
      glassMat.alpha = 1.0;
      glassMat.specularColor = new BABYLON.Color3(0.2, 0.2, 0.2);
  }
